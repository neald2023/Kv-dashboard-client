...
const demoVehicles = [
  ["UTV‑12","RZR XP","Available", "1,240 mi"],
  ["PWC‑07","Sea‑Doo Spark","Out (booking #1024)", "89 hrs"],
];
const demoCustomers = [
...
/* Power Sports demo data */
const demoJetSkis = [
  ["PWC‑01","Sea‑Doo Spark","Available","82 hrs"],
  ["PWC‑07","Sea‑Doo Spark","Out","89 hrs"],
  ["PWC‑11","Yamaha VX","Maintenance","104 hrs"],
];
const demoScooters = [
  ["SCO‑03","Honda Ruckus","Available","1,240 mi"],
  ["SCO‑08","Vespa Primavera","Out","3,420 mi"],
  ["SCO‑10","Honda Metropolitan","Available","780 mi"],
];

/* ---------- Calendar ---------- */
function makeMonthGrid(y,m){ const f=new Date(y,m,1), s=new Date(f); s.setDate(f.getDate()-((f.getDay()+6)%7)); const a=[]; for(let i=0;i<42;i++){const d=new Date(s); d.setDate(s.getDate()+i); a.push(d);} return a; }
function Calendar(){
  ...
  return (
    <div className="w-full max-w-4xl">
      ...
      <div className="grid grid-cols-7 text-sm text-gray-500 mb-1">{["Mon","Tue","Wed","Thu","Fri","Sat","Sun"].map(d=><div key={d} className="p-2">{d}</div>)}</div>
...
function TimeClock(){
  ...
  const total=useMemo(()=>{ let ms=0; for(let i=0;i<rows.length;i+=2){const iIn=rows[i], iOut=rows[i+1]; if(iIn?.action==="in"){ms+=(iOut?.ts ?? Date.now())-iIn.ts;}} return (ms/36e5).toFixed(2); },[rows]);
  return (
...
/* ---------- Power Sports (Jet Skis / Scooters) ---------- */
function PowerSports(){
  const [tab,setTab]=useState("jetSkis"); // "jetSkis" | "scooters"
  const cols = ["ID","Model","Status","Usage"];

  return (
    <div className="w-full max-w-4xl space-y-4">
      <div className="flex gap-2">
        <Button className={tab==="jetSkis"?"bg-emerald-500 text-white border-emerald-600":""} onClick={()=>setTab("jetSkis")}>
          Jet Skis
        </Button>
        <Button className={tab==="scooters"?"bg-emerald-500 text-white border-emerald-600":""} onClick={()=>setTab("scooters")}>
          Scooters
        </Button>
      </div>
      <Table cols={cols} rows={tab==="jetSkis" ? demoJetSkis : demoScooters} />
    </div>
  );
}

/* ---------- App Shell with ALL tabs ---------- */
          ...
          <nav className="flex gap-2 flex-wrap">
            {tabs.map(([k,l])=>(
              <button key={k} onClick={()=>setTab(k)}
                className={"px-3 py-1.5 rounded-full text-sm border transition "+(tab===k?"bg-emerald-500 text-black border-emerald-500":"bg-white text-gray-800 border-gray-300 hover:bg-gray-50")}>
                {l}
              </button>
            ))}
          </nav>
      ...
      <main className="max-w-6xl mx-auto p-4 space-y-6">
        {tab==="dashboard" && (
          <div className="grid md:grid-cols-3 gap-4">
            ...
            <Card title="Modules">Calendar • Clock • Chat • Power Sports</Card>
        ...
        {tab==="vehicles" && <Table cols={["ID","Model","Status","Usage"]} rows={demoVehicles} />}
        ...
        {tab==="powersports" && <PowerSports />}   {/* NEW */}
      </main>
    </div>
  );
}
