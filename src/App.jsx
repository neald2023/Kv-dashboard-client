...
const Pill = ({ tone = "default", children }) => {
  const map = { ok:"bg-green-700/30 text-green-200", warn:"bg-yellow-700/30 text-yellow-100", bad:"bg-red-700/30 text-red-100", default:"bg-slate-700/40 text-slate-200" };
...
export default function App() {
  ...
  const Calendar = () => {
    ...
    (bookings || []).forEach(b=>{
      ...
      if (isValid(s)) {
        const ks = toKey(s); const m = marks.get(ks) || {pickup:[], return:[], ongoing:[]};
        m.pickup.push(b); marks.set(ks,m);
      }
      if (isValid(e)) {
        const ke = toKey(e); const m = marks.get(ke) || {pickup:[], return:[], ongoing:[]};
        m.return.push(b); marks.set(ke,m);
      }
      if (isValid(s) && isValid(e)) {
        const a = s<e ? s : e; const z = s<e ? e : s;
        const walk = new Date(a.getFullYear(), a.getMonth(), a.getDate()+1);
        while (walk < new Date(z.getFullYear(), z.getMonth(), z.getDate())) {
          const kk = toKey(walk); const m2 = marks.get(kk) || {pickup:[], return:[], ongoing:[]};
          m2.ongoing.push(b); marks.set(kk,m2); walk.setDate(walk.getDate()+1);
        }
      }
    });
                        ...
                        <div className="hidden group-hover:block absolute bottom-3 left-1/2 -translate-x-1/2 z-10 w-56 p-2 rounded bg-slate-800 shadow-lg border border-slate-700">
                          ...
                          {mm.ongoing.slice(0,3).map((b,i)=>(
                            <div key={i} className="mt-1 text-xs">
                              <div className="font-medium">{b.customer || b.customerName || b.customerId}</div>
                              <div className="opacity-70">{b.vehicle || b.vehicleName || b.vehicleId}</div>
                              <div className="opacity-50">{new Date(b.start ?? b.startDate).toLocaleDateString()} → {new Date(b.end ?? b.endDate).toLocaleDateString()}</div>
                            </div>
                          ))}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {!vehicles.length && (
                <tr><td className="py-6 text-slate-400" colSpan={5}>No vehicles yet. Click <b>+ Add Vehicle</b> to create one.</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {visible.map((c)=>(
                <tr key={c.id} className="border-b border-slate-800 hover:bg-slate-800/40 cursor-pointer" onClick={()=>openRow(c)}>
                  ...
                  <td className="py-2 pr-4">
                    {c.insurance?.carrier ? (<span>{c.insurance.carrier} • {c.insurance.policyNumber || "—"} {c.insurance.expiresAt ? (<span className="opacity-70">({c.insurance.expiresAt})</span>) : null}</span>) : "—"}
                  </td>
                  <td className="py-2 pr-2 text-right">
              ...
              {!visible.length && (<tr><td className="py-6 text-slate-400" colSpan={6}>No customers. Click <b>+ Add Customer</b> to create one.</td></tr>)}
            </tbody>
          </table>
        </div>
